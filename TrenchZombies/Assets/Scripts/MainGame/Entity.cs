...
public class Entity : MonoBehaviour
{
    ...
    {
        inventory.Add(item);
        itemDatas.Add(item.GetComponent<ItemData>());
    }
    ...
    public void dropItem()
    {
        ...
        if (equipItem)
        {
            ...
            itemDatas.RemoveAt(currentItem - 1);
    ...
    public void scrollItem(int direction)
    {
        ...
        if (equipItem)
        {
            ...
            if (newCurrentItem < 1)
                newCurrentItem = 1;
            else if (newCurrentItem > inventory.Count)
                newCurrentItem = inventory.Count;
    ...
    public void Damage(int dmg)
    {
        ...
        if (Health <= 0)
        {
            transform.GetComponent<PlayerMovement>().mainGame.onKilled(); //Tell the game that the player have died!
        }
    }
